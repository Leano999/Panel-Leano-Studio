...
// Socket.IO only means the panel can reach the server. TikTok status comes from tiktok.
...
socket.emit('tiktok', { username, signApiKey });
...
socket.on('tiktok', ({ connected, username, error, roomId }) => {
if(connected){
...
b.onclick = () => {
socket.emit('trigger', { kind:'sound', id.id });
...
if(id==='clap'){ for(let i=0;i<3;i++) tone(200+Math.random()200, i0.08, 0.06, 'square', 0.15); }
...
else if(id==='drum'){ for(let i=0;i<10;i++) tone(80+i4, i0.05, 0.08, 'triangle', 0.15); }
...
const settings=currentTtsSettings();
return {text(text).slice(0,500), rate.rate, pitch.pitch, volume.volume};
}

async function fetchTtsAudio(text){
const r=await fetch('/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body.stringify(ttsPayload(text))});
if(!r.ok){
...
function setChatStyle(style){ chatStyleSettings.style=style; applyChatStyleUI(); socket.emit('overlay',{ chat }); }
function setChatGap(value){ chatStyleSettings.gap=Number(value)||16; applyChatStyleUI(); socket.emit('overlay',{ chat }); }
function saveChatStyle(){ socket.emit('overlay',{ chat }); logEvent(Style chat disimpan: ${chatStyleSettings.style}, gap ${chatStyleSettings.gap}px); }

function setTheme(theme){
document.querySelectorAll('.theme-card').forEach(el => el.classList.toggle('active', el.dataset.theme === theme));
socket.emit('overlay', { theme });
logEvent(Tema overlay diubah: ${theme});
}

socket.on('overlay', (settings) => {
...
function resetStats(){ socket.emit('stats'); logEvent('Statistik sesi di-reset.'); }
function saveGoal(){
socket.emit('goal', {
...
let actionRules = [];
function addAction(){
actionRules.push({id: Math.random().toString(36).slice(2,9), enabled, event:'comment', keyword:'', action:'tts', value:'Terima kasih {username}!' });
renderActions();
}
...
socket.emit('actions', actionRules);
logEvent(${actionRules.length} custom action rules disimpan.);
}
socket.on('actions', actions => { actionRules = Array.isArray(actions) ? actions : []; renderActions(); });

// ---- Follow/Gift animation settings ----
let animationSettings = { enabled, followStyle:'wings', giftStyle:'treasure', followDuration:4, giftDuration:5, position:'top-center' };
...
socket.emit('overlay', { theme: currentTheme, animations: animationSettings });
...
events:{
onReady(){
try{ chromeMusicPlayer.setVolume(Math.round(chromeMusicVolume100)); try{ chromeMusicPlayer.setPlaybackQuality('tiny'); }catch(e){} }catch(e){}
...
const id=chromeMusicCurrentId; chromeMusicCurrentId=null; chromeMusicAltQueue=[]; socket.emit('music',{videoId});
}
},
onError(){
...
try{
chromeMusicPlayer.setVolume(Math.round(chromeMusicVolume100));
chromeMusicPlayer.loadVideoById({videoId,startSeconds:0,suggestedQuality:'tiny'});
...
if(chromeMusicCurrentId){ const id=chromeMusicCurrentId; chromeMusicCurrentId=null; socket.emit('music',{videoId}); }
}
}
});
}
...
events:{
onReady(){
try{ ytAutoPlayer.setVolume(Math.round(chromeMusicVolume100)); }catch(e){}
...
try{
chromeMusicPlayer.setVolume(Math.round(chromeMusicVolume100));
chromeMusicPlayer.loadVideoById({videoId.videoId,startSeconds:0,suggestedQuality:'tiny'});
setTimeout(()=>{ try{ chromeMusicPlayer.setPlaybackQuality('tiny'); }catch(e){} }, 1200);
chromeMusicPlayer.playVideo();
...
const m = text.match(/(?.com/(??v=|shorts/|live/)|youtu.be/)([a-zA-Z0-9_-]{6,})/);
return m ? m[1] : null;
}
...
socket.emit('ytauto', {query: raw, limit: searchLimit}, function(res){
...
socket.on('music',renderMusic);
socket.on('music',s=>{ chromeMusicVolume=Math.max(0,Math.min(1,Number(s?.volume ?? 0.75))); const el=document.getElementById('musicVolume'); if(el) el.value=Math.round(chromeMusicVolume100); const v=document.getElementById('musicVolumeValue'); if(v) v.textContent=Math.round(chromeMusicVolume100)+'%'; if(chromeMusicPlayer&&chromeMusicApiReady){try{chromeMusicPlayer.setVolume(Math.round(chromeMusicVolume*100));}catch(e){}} });
...
socket.on('music',r=>{ logEvent(r?.ok?Music request masuk: ${r.item?.title||'Lagu'}:Music request gagal: ${r?.message||'error'}); });
...
function musicSkip(){socket.emit('music');logEvent('Music: skip.');}
function musicClear(){socket.emit('music');logEvent('Music: queue dihapus.');}
function musicStop(){socket.emit('music');logEvent('Music: stop dan queue dihapus.');}
function testMusicRequest(){const q=document.getElementById('musicTestQuery').value.trim();if(!q)return;if(!chromeMusicActivated) activateChromeMusic();socket.emit('music',{username:'PenontonDemo',query});}

// ---- Simulator ----
function simulateTtsComment(){const username=document.getElementById('simUsername').value||'PenontonDemo';const comment=document.getElementById('simComment').value||'halo, tes TTS!';socket.emit('trigger',{kind:'alert',type:'comment',username,extra});logEvent('Tes Chat + TTS dikirim ke Browser Source ALERT + TTS. Pastikan audio Browser Source tidak di-mute.');}
...
socket.emit('trigger', {kind:'alert', type, username, extra, count==='like'?5:1});
...
if(statusEl) observer.observe(statusEl,{childList,characterData,subtree});
